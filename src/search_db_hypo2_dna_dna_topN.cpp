...
int make_reverse_complement(int qlen,int n1)
{
  int start = n1-qlen, end = n1-1,i;

  for(i = 0; end >= start; end--, n1++,i++){
    dna[n1] = complement[dna[end]];
    map_input_seq[n1][0] = total_input_seq;
    map_input_seq[n1][1] = i;
  }
  ...
  while(fgets(name,LINE_LEN,f)!=NULL) 
      ...
      if (name[0] == '>')
	{
	  length_each_seq = n-pren;
          ...
          if (type == 1) {
	    input_query_length.push_back(length_each_seq);
	  }
	  else
	    input_DB_length.push_back(length_each_seq);

	for(i = 0; pren<n; pren++,i++){
            map_input_seq[pren][0] = total_input_seq;
            map_input_seq[pren][1] = i;
            dna[pren] = convertStr[dna[pren]];
          }
	  
	if (type == -1)
	    {
	      total_input_seq++;
	      n = make_reverse_complement(length_each_seq,n);
	    }
...
void best_exact_match_dna(int n, FILE *fw2)  {
  ...
  for(i=1;i<n;i++) {
	...
	SA_bottom_db = map_input_seq[SA[i]][0];
      ...
      if(SA_top_db!=-1 && SA_bottom_db!=-1) {        
        ...
        if (input_DB_length[SA_top_db/2] - map_input_seq[SA[start_i-1]][1] < score_up)
          score_up = input_DB_length[SA_top_db/2] - map_input_seq[SA[start_i-1]][1];
	...
	score_bottom = LCP[stop_i+1];
        if (input_DB_length[SA_bottom_db/2] - map_input_seq[SA[stop_i+1]][1] < score_bottom)
          score_bottom = input_DB_length[SA_bottom_db/2] - map_input_seq[SA[stop_i+1]][1];
  ...
  for ( i=0; i<total_qry; i++) {
    ...
    // Do the processing.
    for(k = 0;k<TOP_FREQ; k++){
    	if (freq_db[k].db_id % 2 ==0)
