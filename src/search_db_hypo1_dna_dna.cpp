...
To Do - 
-----
1. map_input_seq[i][2] = length of the sequence -> we could consider just another array to put the length of all sequences instead of data redundency
...
int make_reverse_complement(int qlen,int n1)
{
  ...
  for(i = 0; end >= start; end--, n1++,i++){
    ...
    map_input_seq[n1] = total_input_seq;
  }
  return n1;
}
...
void best_exact_match_dna(int n, FILE *fw2)  {
  int i, j, qry_id, SA_top_db, SA_bottom_db;
  unsigned total_freq;
  ...
  for ( i=0; i<total_qry; i++) {
    ...
    if(count > most_freq_db.freq) {
      most_freq_db.freq = count;      most_freq_db.db_id = prev_db;
    }
